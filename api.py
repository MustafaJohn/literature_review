...
class FetchRequest(BaseModel):
    ...
    input_mode:     str = Field(default="topic", pattern="^(topic|link)$")
...
def run_fetch(req: FetchRequest):
   ...
   @@ -32,55 +32,57 @@ from fastapi.middleware.cors import CORSMiddleware
from fastapi.responses import HTMLResponse
from fastapi.staticfiles import StaticFiles
from pydantic import BaseModel, Field
...
class ClusterItem(BaseModel):
    theme:          str
    description:    str
@@ -97,94 +99,112 @@ class SummarizeRequest(BaseModel):
    papers:         list[dict[str, Any]]
    clusters:       list[dict[str, Any]]
...
def run_fetch(req: FetchRequest):␊
    ...
    input_mode = req.input_mode or "topic"
    if input_mode == "link":
        query = (req.reference_link or req.query or "").strip()
        if not query:
            raise HTTPException(
                status_code=422,
                detail="reference_link is required when input_mode='link'.",
            )
    else:
        ...
        if len(query) < 3:
            raise HTTPException(
                status_code=422,
                detail="query must be at least 3 characters when input_mode='topic'.",
            )

    logger.info("Fetch request: %s [%s] mode=%s max_results=%d sort_by=%s",
                query, req.citation_style, input_mode, req.max_results, req.sort_by)
    t0 = time.time()
    ...
    try:
      ...
      logger.exception("Fetch failed: %s", query)
